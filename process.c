...
unsigned long getUptime(){
	...
	if(!uptime_file){
		fprintf(stderr, "%s: Errore Apertura File Uptime: %s\n", __func__, strerror(errno));
		return -1;
	}
	int ret = fscanf(uptime_file, "%lu", &uptime);
	if(ret == EOF){
...
unsigned long long getStatTime(){
	/*
       /proc/stat
...
//	long ticks = sysconf(_SC_CLK_TCK);	//numero clock ticks al secondo
	...
	*/
	...
	ret = fscanf(file1, format_string, &pid, &comm, &state, &flags, &utime1, &stime1, &starttime, &rss);
	if(ret==EOF){
	...
	fclose(file1);
	/*
	long unsigned uptime1 = getUptime();
	if(uptime1 == -1){
		fprintf(stderr, "%s: Errore Uptime1.\n", __func__);
		return -1;
	}
	*/
	long long unsigned tot_time1 = getStatTime();
	if(tot_time1 == -1){
		fprintf(stderr, "%s: Errore tot_time1.\n", __func__);
		return -1;
	}
///////
	usleep(10000);
///////
	...
	/*
	unsigned long uptime2 = getUptime();
	if(uptime2 == -1){
		fprintf(stderr, "%s: Errore Uptime2.\n", __func__);
		return -1;
	}
	*/
	long long unsigned tot_time2 = getStatTime();
	if(tot_time2 == -1){
		fprintf(stderr, "%s: Errore tot_time2.\n", __func__);
		return -1;
	}

	long long unsigned time1 = utime1 + stime1;
	long long unsigned time2 = utime2 + stime2;
	//long long unsigned uptime = (uptime2 - uptime1) * ticks;
	long long unsigned interval = tot_time2 - tot_time1;

	printf("Processo %d - tempo usato: %llu, tempo passato: %llu .\n", pid, time2-time1, interval);

	float cpu_usage;
	if(interval)
		cpu_usage =  100 * (time2 - time1) / interval;
	else  cpu_usage = -1.234;
